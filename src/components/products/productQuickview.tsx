          ...
          <div className="modal-content">
            <div className="card card-product">
              <div className="card-body d-block d-lg-flex p-4">
                <button
                  type="button"
                  className="btn-close text-dark position-absolute me-4 end-0"
                  data-bs-dismiss="modal"
                  aria-label="Close"
                ></button>
                {thumb_src && (
                  <img
                    className="w-70 w-md-60 w-lg-30 rounded-3 shadow-xs border mb-4 mb-md-0"
                    src={thumb_src}
                  />
                )}
                <div className="w-100 w-lg-70 ps-4 mt-md-5 mt-lg-0">
                  <div className="d-flex align-items-center justify-content-between">
                    {title.length != 0 && <h4 className="mb-0">{title}</h4>}
                  </div>
                  {price && (
                    <>
